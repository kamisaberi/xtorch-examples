...
public:
    ...
    Target get(size_t index) override
    {
        ...
        for (int i = 0; i < num_objects; ++i)
        {
            ...
            mask.index_put_({
                                torch::indexing::Slice(int(y1), int(y1 + h)),
                                torch::indexing::Slice(int(x1), int(x1 + w))
                            }, 1);
...
struct SimplifiedMaskRCNNImpl : torch::nn::Module
{
    ...
    SimplifiedMaskRCNNImpl(int num_classes = NUM_CLASSES)
    {
        ...
        box_regressor = register_module("box_regressor", torch::nn::Linear(roi_feat_dim, num_classes * 4));
        // Per-class regression or class-agnostic (num_classes=1 or just 4)
        ...
        mask_predictor = register_module("mask_predictor", torch::nn::Conv2d(128, num_classes, 1));
        // Predict one mask per class (or 1 for class-agnostic)
    }
    ...
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> forward(
        torch::Tensor images,
        const std::optional<std::vector<torch::Tensor>>& proposals_batch_opt = {}
        // [num_proposals, 4] (x1,y1,x2,y2) pixel coords
    )
    {
        ...
        if (proposals_batch_opt.has_value())
        {
            ...
            for (int i = 0; i < batch_size; ++i)
            {
                ...
                torch::Tensor batch_idx_col = torch::full({proposals_for_image.size(0), 1}, i,
                                                          proposals_for_image.options());
        ...
        {
            // Fallback: simulate some random proposals if none provided (e.g., for inference)
            for (int i = 0; i < batch_size; ++i)
            {
                int num_rand_props = 10;
                torch::Tensor rand_x1y1 = torch::randint(0, IMG_SIZE / 2, {num_rand_props, 2},
                                                         features.options().dtype(torch::kFloat));
                torch::Tensor rand_wh = torch::randint(10, IMG_SIZE / 2, {num_rand_props, 2},
                                                       features.options().dtype(torch::kFloat));
        ...
        if (total_rois == 0 || rois_list.empty())
        {
        ...
        torch::Tensor mask_logits = mask_predictor->forward(x_mask);
        // [TOTAL_ROIS, NUM_CLASSES, MASK_OUTPUT_SIZE, MASK_OUTPUT_SIZE]
...
)
{
    ...
    for (size_t i = 0; i < targets_batch.size(); ++i)
    {
        ...
        {
            torch::Tensor gt_label_for_rois = gt_target.labels[0].repeat({rois_for_item_indices.size(0)});
            // [NumRoIs_for_item]
            torch::Tensor gt_box_for_rois = gt_target.boxes[0].unsqueeze(0).repeat({rois_for_item_indices.size(0), 1});
            // [NumRoIs_for_item, 4]

            // Dummy mask target: resize GT mask to MASK_OUTPUT_SIZE
            torch::Tensor gt_mask_for_rois = gt_target.masks[0].unsqueeze(0).unsqueeze(0).to(torch::kFloat);
            // [1, 1, H, W]
            ...
            gt_mask_for_rois = gt_mask_for_rois.repeat({
                rois_for_item_indices.size(0), gt_target.labels[0].item<long>(), 1, 1
            }); // [NumRoIs_for_item, C_idx, MASK_H, MASK_W]
            ...
            torch::Tensor current_mask_logits = mask_logits.index_select(0, rois_for_item_indices);
            // Shape [N_rois, NC, MH, MW]
            ...
            torch::Tensor relevant_mask_logits = current_mask_logits.select(1, 1);
            // Select logits for class 1 [N_rois, MH, MW]

            // Dummy losses
            total_loss += torch::nll_loss(torch::log_softmax(current_class_logits, 1), gt_label_for_rois);
            total_loss += torch::mse_loss(relevant_box_preds, gt_box_for_rois.to(relevant_box_preds.device()));
            // Should be SmoothL1
            ...
            gt_mask_resized_for_loss = torch::nn::functional::interpolate(
                gt_mask_resized_for_loss.unsqueeze(0), // [1,1,H,W]
                torch::nn::functional::InterpolateFuncOptions().size(std::vector<int64_t>{
                    MASK_OUTPUT_SIZE, MASK_OUTPUT_SIZE
                }).mode(torch::kBilinear).align_corners(false)
            ).squeeze(0).squeeze(0); // [MH, MW]
            gt_mask_resized_for_loss = gt_mask_resized_for_loss.repeat({rois_for_item_indices.size(0), 1, 1});
            // [N_rois, MH, MW]

            total_loss += torch::binary_cross_entropy_with_logits(relevant_mask_logits,
                                                                  gt_mask_resized_for_loss.to(
                                                                      relevant_mask_logits.device()));
        }
    }
    ...
    for (int64_t epoch = 1; epoch <= NUM_EPOCHS; ++epoch)
    {
        ...
        for (auto& batch : *train_loader)
        {
            ...
            for (int i = 0; i < targets_batch.size(); ++i)
            {
                ...
                torch::Tensor batch_idx_col = torch::full({t_boxes.size(0), 1}, static_cast<float>(i),
                                                          t_boxes.options().device(device));
            ...
            if (class_logits.size(0) == 0 && box_preds.size(0) == 0 && mask_logits.size(0) == 0 && total_rois_for_loss
                == 0)
            {
                std::cout << "Epoch: " << epoch << " Batch: " << batch_idx << " - No proposals/GTs, skipping loss." <<
                    std::endl;
                batch_idx++;
                continue;
            }
            ...
            {
                // No actual RoIs processed by model
                std::cout << "Epoch: " << epoch << " Batch: " << batch_idx <<
                    " - Model returned no predictions, skipping loss." << std::endl;
                batch_idx++;
                continue;
            }
            ...
            // Compute loss
            torch::Tensor loss = compute_mask_rcnn_loss(class_logits, box_preds, mask_logits, targets_batch,
                                                        rois_for_loss_calc);

            if (loss.defined() && loss.item<double>() > 0)
            {
            ...
            {
                std::cout << "Epoch: " << epoch << " Batch: " << batch_idx <<
                    " - Invalid loss (0 or undefined), skipping backward/step." << std::endl;
            }
