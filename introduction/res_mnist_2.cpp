...
std::vector<float> adapt_resnet_output_to_mnist(const float* resnet_output_data,
                                                ...
                                                size_t mnist_classes) { // Should be NUM_MNIST_CLASSES
    ...
    for (size_t i = 0; i < mnist_classes; ++i) {
        ...
        for (size_t j = 0; j < resnet_features; ++j) {
         ...
         mnist_adapted_output[i] = sum; // The original effectively summed up all features `resnet_features` times with different random weights for each of the 10 classes.
...
int main() {
    ...
    try {
        ...
        if (resnet_output_shape.size() != 2 || resnet_output_shape[0] != 1 || resnet_output_shape[1] != RESNET_OUTPUT_FEATURES) {
            std::cerr << "Error: Unexpected ResNet output shape. Expected [1, " << RESNET_OUTPUT_FEATURES << "]." << std::endl;
            return -1;
        }
